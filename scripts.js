document.addEventListener('DOMContentLoaded', function() {
  document.querySelector('.range-form').addEventListener('submit', function(event) {
    ...
    const fullBarcode = barcodeInput.value;
    const numericPart = fullBarcode.split('-').slice(1).join('-');

    document.querySelector('#barcode p').textContent = fullBarcode;
    document.querySelector('#box-number p').textContent = numericPart;
  ...
  barcodeInput.addEventListener('input', function() {
    const query = barcodeInput.value.toLowerCase();
    autocompleteContainer.innerHTML = '';

    if (query) {
      const suggestions = boxes_array.filter(box => box.box_number.toLowerCase().includes(query));

      suggestions.forEach(suggestion => {
        const suggestionElement = document.createElement('div');
        suggestionElement.textContent = suggestion.box_number;
        suggestionElement.classList.add('autocomplete-suggestion');

        suggestionElement.addEventListener('click', function() {
          barcodeInput.value = suggestion.box_number;
          autocompleteContainer.innerHTML = '';
        });
